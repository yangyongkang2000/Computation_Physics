...
vector<array<T,2>> runge_kutta(const function_type&func,const pair<T,T> &_cond,const T &_end,const T &h=1e-2)
{
    if(h<=0)
        return {};
    T k1,k2,k3,k4,u_1,u_2;
    vector<array<T,2>> result {{_cond.first,_cond.second}};
    if(_cond.first>=_end)
        for(T x=_cond.first,y=_cond.second;x>=_end;x-=h)
         {
             k1=func(x,y);
...
vector<pair<T,vector_type>> runge_kutta(const function_vector_type &func_vector, pair<T,vector_type>_cond,const T& _end,const T &h=1e-2)
{
   if(h<=0)
    ...
    T k1,k2,k3,k4;
    if(_cond.first>=_end)
        for(T x=_cond.first;x>=_end;x-=h)
         {
...
vector<pair<T, vector_type>> desolve(const function_type& func,pair<T,vector_type> _cond,const T& _end,const T &h=1e-2)
{
    ...
    return runge_kutta(func_vector, _cond, _end,h);
}
